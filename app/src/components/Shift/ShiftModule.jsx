...
export const ShiftModule = () => {
  ...
  const handleTransaction = async (e, type) => {
    ...
    try {
      ...
      const cbGenerado = (type === 'egreso' && (cuenta.tipo === 'Crédito' || cuenta.tipo === 'Débito') && parseFloat(txForm.porcentaje_cashback) > 0) 
        ? monto * (parseFloat(txForm.porcentaje_cashback) / 100) : 0;
    ...
    <div className="w-full text-white font-sans relative pb-32 animate-in fade-in duration-700">
      ...
      {viewState === 1 && (
        <div className="max-w-xl mx-auto space-y-10 animate-in slide-in-from-bottom-8 duration-500">
          <div className="bg-black/20 backdrop-blur-2xl border border-white/5 rounded-[3rem] p-6 md:p-10 relative overflow-hidden group shadow-2xl">
      ...
      {viewState === 2 && activeShift && (
        <div className="max-w-xl mx-auto space-y-6 animate-in zoom-in-95 duration-500">
          <div className={`backdrop-blur-3xl border rounded-[3rem] p-8 md:p-10 text-center relative overflow-hidden transition-all duration-700 ${activeShift.estado === 'Pausada' ? 'bg-orange-500/10 border-orange-500/30 shadow-[0_20px_50px_rgba(249,115,22,0.1)]' : 'bg-black/40 border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]'}`}>
            ...
            <div className="flex justify-between items-start w-full relative z-10 mb-4">
                ...
                {activeShift.lat_inicio && (
                   <div className="flex items-center gap-1 text-[8px] text-white/30 uppercase tracking-widest font-black"><Navigation size={10} className="text-cyan-500" /> GPS OK</div>
                )}
            </div>
          ...
          {activeShift.estado === 'Pausada' ? (
              <button onClick={handleTogglePause} disabled={isSubmitting} className="w-full py-12 bg-orange-500 text-white font-black uppercase tracking-widest text-sm rounded-[2.5rem] active:scale-95 transition-all shadow-[0_10px_40px_rgba(249,115,22,0.3)] flex flex-col justify-center items-center gap-3 animate-in fade-in duration-300">
          ...
          <div className="grid grid-cols-3 gap-2 text-center bg-black/40 backdrop-blur-xl p-5 rounded-3xl border border-white/5 animate-in slide-in-from-bottom-4 duration-500">
            <div>
      ...
      {showChecklist && (
        <div className="fixed inset-0 z-100 flex items-end md:items-center justify-center bg-black/80 backdrop-blur-md p-0 md:p-4 overflow-hidden animate-in fade-in duration-300">
            ...
            <div className="space-y-3 mb-8">
                {checklistDB?.map((item) => (
                    <div 
                        ...
                        className={`p-5 rounded-2xl flex justify-between items-center cursor-pointer transition-all border ${checkedItems[item.id] ? 'bg-emerald-500/10 border-emerald-500/30' : 'bg-white/5 border-white/5'}`}
                    >
      ...
      {/* NUEVO MODAL: REPOSTAJE DE COMBUSTIBLE */}
      {showFuelModal && (
        <div className="fixed inset-0 z-100 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-md p-0 md:p-4 overflow-hidden animate-in fade-in duration-300">
          <div className="absolute inset-0" onClick={() => { triggerHaptic('light'); setShowFuelModal(false); }}></div>
          <div className="w-full max-w-xl bg-[#0A0A0A]/95 backdrop-blur-3xl border-t border-white/10 rounded-t-[2.5rem] md:rounded-[2.5rem] p-8 md:p-10 shadow-[0_-20px_50px_rgba(0,0,0,0.8)] relative animate-slide-up-sheet max-h-[90dvh] overflow-y-auto z-10 pb-16 md:pb-10">
            ...
            <button onClick={() => { triggerHaptic('light'); setShowFuelModal(false); }} className="absolute top-6 right-6 text-white/40 hover:text-white bg-white/5 p-2 rounded-full hidden md:block active:scale-90"><X size={20} /></button>
            <h2 className="text-3xl font-black tracking-tighter text-white mb-8 flex items-center gap-3"><Fuel className="text-yellow-500" /> Repostaje</h2>
            ...
            <form onSubmit={handleFuelSubmit} className="space-y-5">
              ...
              <div className="space-y-1.5 pt-2">
                ...
                <select required value={fuelForm.cuenta_id} onChange={e => setFuelForm({...fuelForm, cuenta_id: e.target.value})} className="w-full bg-white/5 border border-white/5 text-white font-bold p-4 rounded-2xl outline-none appearance-none focus:border-white/30">
                  <option value="" disabled className="bg-black text-white/40">Pagado con...</option>
                  {cuentas?.map(c => <option className="bg-black" key={c.id} value={c.id}>{c.nombre_cuenta} ({c.tipo})</option>)}
                </select>
              </div>

              <div className="space-y-1.5 pt-2">
                <label className="text-[9px] text-white/40 font-bold uppercase tracking-widest ml-1">Costo Total ($)</label>
                <input type="number" step="0.01" autoFocus required value={fuelForm.costo_total} onChange={e => setFuelForm({...fuelForm, costo_total: e.target.value})} className="w-full bg-white/5 border border-white/5 text-yellow-500 text-3xl font-black p-5 rounded-2xl outline-none font-mono focus:bg-white/10 transition-all" placeholder="0.00" />
              </div>
      ...
      {/* MODALES RÁPIDOS DE TRANSACCIÓN (INGRESO / GASTO) */}
